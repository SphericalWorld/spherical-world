...
export default class MouseSource implements InputSource {
  ...
  onMove = (e: MouseEvent): void => {
    this.dispatch(new RangeInputEvent(MOUSE_MOVE, e.movementX, e.movementY));
  };

  onMouseDown = (e: MouseEvent): void => {
    this.dispatch(new StateInputEvent(keys[e.button], STATE_DOWN));
  };

  onMouseUp = (e: MouseEvent): void => {
    this.dispatch(new StateInputEvent(keys[e.button], STATE_UP));
  };
  ...
  changeTracking = (): void => {
    if (document.pointerLockElement) {
      ...
      this.setClickHandlers(document, this.canvas);
      this.dispatch(new StateInputEvent(MOUSE_POINTER_UNLOCKED));
    }
  };
}
