...
export const getBlock = (terrain) => (x: number, y: number, z: number): Maybe<Block> =>
  terrain
    .getChunk(toChunkPosition(x), toChunkPosition(z))
    .map((chunk) => chunk.getBlock(toPositionInChunk(x), Math.floor(y), toPositionInChunk(z)));
 ...
 */
export const getLight = (terrain) => (x: number, y: number, z: number): Maybe<vec3> =>
  terrain
    .getChunk(toChunkPosition(x), toChunkPosition(z))
    .map((chunk) =>
      getColorComponents(
